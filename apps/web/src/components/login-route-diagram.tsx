...
function FlatNode({
  ...
  return (
    <g>
      <rect x={x - 34} y={y - 19} width="68" height="48" rx="12" fill="var(--route-shadow)" />
      <rect
        x={x - 34}
        y={y - 24}
        width="68"
...
export function FlatLoginRouteDiagram() {
  return (
    <svg
      viewBox="0 0 760 440"
      className="login-route-diagram h-full w-full"
      role="img"
      aria-label="Coding agents route through OpenMetal to reach sandbox providers"
    >
      <defs>
        <pattern id="loginRouteFlatGrid" width="32" height="32" patternUnits="userSpaceOnUse">
          <path d="M 32 0 L 0 0 0 32" fill="none" stroke="var(--route-grid)" strokeWidth="1" />
        </pattern>
        <radialGradient id="loginRouteFlatGlow">
          <stop offset="0%" stopColor="var(--route-hub-glow)" stopOpacity="0.12" />
          <stop offset="100%" stopColor="var(--route-hub-glow)" stopOpacity="0" />
        </radialGradient>
        <radialGradient id="loginRouteFlatFade">
          <stop offset="0%" stopColor="#fff" stopOpacity="1" />
          <stop offset="65%" stopColor="#fff" stopOpacity="0.7" />
          <stop offset="100%" stopColor="#fff" stopOpacity="0" />
        </radialGradient>
        <mask id="loginRouteFlatMask">
          <ellipse cx="380" cy="220" rx="380" ry="220" fill="url(#loginRouteFlatFade)" />
        </mask>
      </defs>

      <g mask="url(#loginRouteFlatMask)">
        <rect width="760" height="440" fill="url(#loginRouteFlatGrid)" />
      </g>

      {AGENTS.map((agent) => (
        <path
          key={`agent-base-${agent.label}`}
          d={`M ${agent.x + 34} ${agent.y} H 230 L 320 216`}
          fill="none"
          stroke="var(--route-line)"
          strokeWidth="1.5"
          strokeLinecap="round"
      ...
      {SANDBOXES.map((sandbox) => (
        <path
          key={`sandbox-base-${sandbox.label}`}
          d={`M 440 216 L 540 ${sandbox.y} H ${sandbox.x - 34}`}
          fill="none"
          stroke="var(--route-line)"
          strokeWidth="1.5"
          strokeLinecap="round"
      ...
      {SANDBOXES.map((sandbox, index) => (
        <path
          key={`sandbox-signal-${sandbox.label}`}
          d={`M 440 216 L 540 ${sandbox.y} H ${sandbox.x - 34}`}
          fill="none"
          stroke="var(--route-signal)"
          strokeWidth="4"
          ...
          strokeLinejoin="round"
          className="hero-route-flow"
          style={{ animationDelay: `${(index + 0.5) * -0.8}s` }}
        />
      ))}

      {AGENTS.map((agent) => (
        <FlatNode key={agent.label} x={agent.x} y={agent.y} label={agent.label}>
          {agent.mark}
        </FlatNode>
      ))}
      ...
      <circle
        ...
        cy="216"
        r="96"
        fill="url(#loginRouteFlatGlow)"
        className="hero-route-pulse"
      />
      <rect x="320" y="190" width="120" height="80" rx="18" fill="var(--route-shadow)" />
      <rect
        x="320"
        y="182"
        width="120"
