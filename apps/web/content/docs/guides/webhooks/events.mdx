...
description: Every subscribable Metal event type with signed payload examples.
---
...
| `sandbox.attempt_started` | reserved, not currently emitted                              |
| `sandbox.attempt_failed`  | reserved, not currently emitted                              |
...
Worker-emitted sandbox and runtime events always include `sandbox_id` alongside the
